...
for x, y in zip(x_test, y_test):
    ...
    # print(f"Output values (first 10): {output.flatten()[:10]}")
    # print(f"Max value in output: {np.max(output)}")
    # print(f"Min value in output: {np.min(output)}")
    # print(f"Sum of output: {np.sum(output)}")
    # print(f"argmax result: {np.argmax(output)}")
    # print(f"True label shape: {y.shape}")
    # print(f"True argmax: {np.argmax(y)}")
    # print("---")
    if i % 100 == 0:
